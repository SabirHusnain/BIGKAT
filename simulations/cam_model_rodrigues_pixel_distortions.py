...
##Camera parameters
fx, fy = 3.5, 3.5 #Camera focal length
...
def radial_dist(p):    
    ...
    p_norm = (p[:2] - np.array(resolution)/2)/ np.array([fx_p, fy_p])
...
def radial_dist2(p):
    ...
    r = (p[0] - resolution[0]/2)**2 + (p[1] - resolution[1])**2
        
    radial = (k1 * r + k2 * r**2)
    
    output[0] = p[0] + (p[0] - resolution[0]/2) * radial
    output[1] = p[1] + (p[1] - resolution[1]/2) * radial
...
rod = np.hstack((np.random.uniform(-1.25, 1.25, size = (10,2)), np.random.uniform(0, 0.5, size = (10,1))))     
                
trans = np.hstack((np.random.uniform(-80, 80, size = (10,2)), np.random.uniform(150, 800, size = (10,1))))    
...
for rt in range(len(rod)):
    ...
    img_radial_dist_new = np.empty(chessboard.shape, dtype = np.float32) 
    ...
    for c in range(len(chessboard)):   
        ...
        img_radial_dist_new[c] = radial_dist2(imgpointspx_new[c]) #Radial distortion parameter  
...
#        #Plot the chessboards projection onto the camera
        ...
        plt.scatter(img_radial_dist_new[:,0], img_radial_dist_new[:,1], color = 'r')
#        plt.axvline(width/2)
...
##sm = pystan.StanModel(file='bayesianCam_mult_rodrig_pixel_trans_dist.stan') #Compile Stan Model
...
#    return (x-mean) / sd
#    
#init2 = [{'sigma_unif': 1,  'fx_raw': raw_scale(fx_p, 1024, 100), 'fy_raw': raw_scale(fy_p, 1024, 100), 'cx_raw': raw_scale(resolution[0]/2, 360, 50),  'cy_raw': raw_scale(resolution[0]/2, 640, 50), 
#         'r1_raw': raw_scale(rod[:,0], 0, 0.5), 'r2_raw': raw_scale(rod[:,1], 0 ,0.5), 'r3_raw': raw_scale(rod[:,2], 0, 1), 't1_raw': raw_scale(trans[:,0], 0, 50), 
#         't2_raw': raw_scale(trans[:,1], 0, 50), 't3_raw': raw_scale(trans[:,2], 0, 300), 'k1_raw': raw_scale(k1, 0, 0.001), 'k2_raw': raw_scale(k1, 0, 0.001) }]
#
#
#
#fit = sm.sampling(data=stan_data, iter = 10000, chains=4, 
