...
##Camera parameters
fx, fy = 3.6, 3.6 #Camera focal length
cx, cy = 0.5, 0.25 #Offsets of the 
...
RT = get_RT_matrix((np.pi/5, np.pi/4, 0), (100, 200, 300))
...
####Get a load of chessboard images
rotations = np.random.uniform(-0.69813170079773179, 0.69813170079773179, size = (10, 3))
all_imagepointspx = [] #Empty array to put all images points in
all_imagepointspx_stan = []
M = get_camera_matrix(fx, fy, cx, cy, s) #Camera Intrinsic matrix   
i = 0
for rot in rotations:
    ...
    RT = get_RT_matrix(rot, (100, 200, 500))    
    H = get_homography_matrix(M, RT)
    ...
    all_imagepointspx.append(imgpointspx_new[:,:2])
    all_imagepointspx_stan.append(imgpointspx_new)
    if plot_example:   
        fig = plt.figure()
        ax2 = fig.add_subplot(111, projection='3d')
...
all_images = np.array(all_imagepointspx_stan)
