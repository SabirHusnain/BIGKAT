...
##Camera parameters
fx, fy = 4, 3.5 #Camera focal length
cx, cy = 0.0, 0 #Offsets of the camera center 
...
for rt in range(len(rod)):
    ...
    for c in range(len(chessboard)):   
        ...
        imgpointspx_new[c] = mm_to_pixel(imgpoints_new[c], kx, ky, resolution) + np.append(np.random.normal(0, sigma, size = 2), 0) 
...
sm = pickle.load(open('bayesianCam_mult_rodrig_stan.pkl', 'rb'))
#sm = pystan.StanModel(file='bayesianCam_mult_rodrig.stan') #Compile Stan Model
#with open('bayesianCam_mult_rodrig_stan.pkl', 'wb') as f: #Save Stan model to file
#    pickle.dump(sm, f)
