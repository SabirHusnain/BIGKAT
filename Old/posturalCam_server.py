...
import time, io, os, glob, pickle, socket, threading
import picamera
import cv2
...
class posturalCam:
    ...
    def start_UDP_server(self):
        ...
        print('UDP Server: starting server up on {} port {}'.format(server_address[0], server_address[0]))
        ...
        # client_t0 = float(data_decoded[0])
        # server_t0 = time.time()

        # print(server_t0 - client_t0)
    ...
    def start_UDP_client(self, t):
        ...
        sent = sock.sendto('{},{}'.format(time.time(), t).encode(), server_address)  # Send the current time and t)
    ...
    def start_TCP_server(self):
        ...
        host = '196.168.0.2'  # Get local machine name
    ...
    def start_TCP_client(self):
        ...
        while True:
            try:
                ...
                print("TCP Client: Could not connect to server. Trying again")
                i += 0
                if i > 100:
...
def cam2_client():
    myCam = posturalCam()
    myCam.start_UDP_client(5)  # Command server to record for 5 seconds
    myCam.start_TCP_client()


cam1_server()

# thread1 = myThread(1, 'server_cam', cam1_server)
